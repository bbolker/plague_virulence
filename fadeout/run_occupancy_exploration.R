...
outdir <- here::here("fadeout", "output", "patch_occupancy")
...
for (parameter in names(parameter_values)) {
  ...
  ggsave(
    file.path(figdir, sprintf("occupancy_compare_%s.pdf", parameter)),
    p_compare,
    width = 10,
    height = 6
  )
  ggsave(
    file.path(figdir, sprintf("occupancy_compare_%s.png", parameter)),
    p_compare,
    width = 10,
    height = 6,
    dpi = 180
...
write.csv(
  occupancy_data,
  file.path(datadir, "occupancy_curves_absorbing.csv"),
  row.names = FALSE
)
write.csv(
  curve_summaries,
  file.path(datadir, "curve_summaries.csv"),
  row.names = FALSE
)
saveRDS(results, file.path(datadir, "occupancy_results.rds"))
