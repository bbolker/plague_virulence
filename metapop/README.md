...
* Drop the synchrony and just have parallel ODEs. Active outbreaks can spark outbreaks among “neighbors”. In order to do this we would run deterministic SIR ODEs in every patch, running the models (using `lsodar`) until any current epidemic reached its first (?) trough (i.e., stop when `r*N/(1-N/K) - beta*S*I == 0`); at that time, determine whether the patch goes extinct (not sure how this works for a two-strain model: do we just do the branching-process calculation at this point?). Patches where one or both strains are extinct re-ignite with a probability depending on the force of infection coming from other patches. (Given that this force of infection will be continuously varying in time, the root-finding exercise for finding extinction times might be too complicated; we might fall back on a (small-Δt) discrete-time model where we do (1) branching-process exinction-probability calculations in extant patches if prevalence (or incidence?) is below a threshold; (2) re-ignition (fizzle) probability calculations in extinct patches based on current between-patch FOI; (3) deterministic SIR + logistic-growth updates otherwise
   Note that the branching-process calculations (which I haven't yet thought about working out how to use here) determine probability of *eventual* extinction, not probability of extinction in the next Δt, but I think that's OK; the FOI contributed by infections during the branching process before the actual extinction time should be negligible.
   Further thoughts on this:
   * might be worth making the discrete-time step here a *disease generation*: this makes the within-patch model something like a Reed-Frost model
   * hybrid stochastic/deterministic; if prevalence/incidence are above a threshold, use SIR + logistic growth dynamics. If prevalence/incidence are zero, model Poisson introductions. If prev/inc are >0 but less than a threshold, model binomial disease dynamics. (Could treat only extinction as stochastic, i.e. epi dynamics conditional on non-extinction are deterministic, but might as well add the extra noise?)
   * in this non-synchronized case, it might be important to put seasonality back in (see @krauerInfluenceTemperatureSeasonality2021 for useful data/conclusions? Most European epidemics were Aug-Nov ...)
   * if we're being this realistic/brute-force, we can model density-dependence of R0 however we like
