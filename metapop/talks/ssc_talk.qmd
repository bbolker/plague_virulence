...
## tradeoff theory for plague

![](pix/lenski_date.png){.absolute width=373px height=63px left=335px top=97px}
![](pix/lenski_hdr.png){.absolute width=407px height=105px left=-73px top=80px}
![](pix/lenski_text.png){.absolute width=575px height=549px left=-1px top=196px}
...
* **density-dependent $\rzero$**: lower $\rzero$ → more surviving hosts → higher probability of local outbreak (?)
...
* patch state
    * patch occupancy: $\{p_{\varnothing}, p_1, p_2, p_{12},
    N_{\varnothing}, N_1, N_2, N_{12}\}$
    * multilevel: $S_{i}$, $I_{ij}$ $\in {\mathbb Z}$ 
* time
    * discrete ($\Delta t$ = 1 epidemic)
    * discrete ($\Delta t$ = 1 disease generation)
    * continuous

## strategy

* start with single-species metapopulations
* find parameter values where decreasing $R_0$ **increases** patch occupancy/persistence
* $\approx$ parameter values favoured by evolution?
...
* discrete-time, one disease *generation* per step ($\approx$ 15 days)
* continuous-time: Euler steps
...
* **epidemic size**: epidemic either "fizzles" or follows standard dynamics; possibility of burnout
...
## single-strain model (burnout)

![](pix/burnout_sum.png){.absolute width=775px height=598.3px left=0px top=89px}

## single-strain model (colonization)

![](pix/poisson_ext.png){.absolute width=775.2px height=599.8px left=0px top=89px}
...
* effects of *Pla* depletion on alternative hosts: **sylvatic cycle**
