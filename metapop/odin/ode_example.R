...
plot_width <- plot_height <- 6
dt <- 0.01
r <- 0.125 ## was 0.125
run1 <- discrete_run(beta_vec  = c(4, 0), ## was (4, 0)
                     K         = 1,
                     r         = r,
                     n_patch   = 1,
                     nt        = 45/dt, ## round(10 / r / dt),
                     alpha     = 0,
                     I_init    = c(0.001, 0),
                     I_ini_method = "fixed",
                     dt        = dt,
...
plotfun <- function(run) {
  ...
  t1 <- t0 |>
    ...
    filter(!var %in% c("trough_area", "any_small")) |>
...
sfun <- function(ext) {
  ggsave(paste0("ode_trough_example", ext),
