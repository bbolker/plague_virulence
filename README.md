...
- `3d`: Levins model, state variables {average host pop size in S
  ...
  infected} `within_season_transmission`: ? `report_for_summer`: YZ’s
  report `fixedns`: : 2D model Levins model, state variables: {average
  host pop size; fraction of patches infected} `averaging_all_patches`:
  ? `outputs`
