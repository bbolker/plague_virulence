...
curves<-fread('2d_ode_validation/data/psi1_2d_ode_curves.csv')[!is.na(P_kendall)]
...
cols_method<-c('Numerical finite-horizon Kendall'='#009E73','Closed trough saddle'='#CC79A7')
...
draw_probability<-function(kind,rho_value,theta_value){
  if(kind=='conditional'){
    ...
    ttl<-'Conditional persistence probability';s<-copy(sim[rho==rho_value&theta==theta_value])
    ...
    ttl<-'Unconditional persistence probability';s<-copy(sim[rho==rho_value&theta==theta_value])
    s[,`:=`(estimate=P_unconditional,low=uncond_low,high=uncond_high)]
  }
  a<-melt(copy(curves[rho==rho_value&theta==theta_value]),id.vars=c('rho','theta','K','R0'),
    measure.vars=measures,variable.name='method',value.name='value')
  a[,method:=factor(method,measures,c('Numerical finite-horizon Kendall','Closed trough saddle'))]
  legend_stub<-CJ(K=sort(unique(s$K)),method=factor(c('Numerical finite-horizon Kendall',
    'Closed trough saddle'),levels=c('Numerical finite-horizon Kendall','Closed trough saddle')))
  legend_stub[,`:=`(R0=NA_real_,value=NA_real_)]
  p<-ggplot()+geom_line(data=legend_stub,aes(R0-1,value,colour=method),linewidth=.9,na.rm=TRUE)+
    ...
    scale_x_log10(breaks=c(.05,.1,.2,.5,1,2,5))+scale_y_continuous(limits=c(0,1),
      breaks=seq(0,1,.25),expand=expansion(mult=c(.01,.03)))+
    scale_colour_manual(values=cols_method,limits=names(cols_method),drop=FALSE)+
    labs(title=ttl,subtitle=sprintf('Full 2D ODE; psi = 1, rho = %.2f, theta = %g; I0 = 1',rho_value,theta_value),
      ...
      caption='Blue points and grey bars: new CTMC estimates and 95% Wilson intervals.\nGreen: direct finite-horizon Kendall quadrature; purple: closed trough-saddle formula. Curves stop where the finite-horizon interval does not exist.')+theme_2d
...
write_pages('unconditional','2d_ode_validation/figures/fig12_psi1_2d_ode_unconditional.pdf')
write_pages('conditional','2d_ode_validation/figures/fig13_psi1_2d_ode_conditional.pdf')

cmp<-curves[K==1000&R0%in%c(1.5,2,3,6)&status=='OK'&!is.na(P_trough)]
mae<-mean(abs(cmp$P_trough-cmp$P_kendall))
p<-ggplot(cmp,aes(P_kendall,P_trough,colour=factor(rho),shape=factor(theta)))+
  geom_abline(slope=1,intercept=0,colour='grey55',linetype='dashed')+geom_point(size=2.3)+
  ...
  labs(title='Closed trough formula versus numerical Kendall quadrature',
    subtitle=sprintf('%d resolved-trough points; psi = 1, K = 1000; mean absolute difference = %.5f',nrow(cmp),mae),
    x='Numerical finite-horizon Kendall probability',y='Closed trough-saddle probability',
    caption='The diagonal is equality. Only parameter points with a resolved recovery trough are included.')+theme_2d
cairo_pdf('2d_ode_validation/figures/fig15_psi1_2d_ode_trough_vs_kendall.pdf',7.4,7.0,onefile=FALSE)
print(p);dev.off();cat('psi=1 2D ODE Figures 12, 13, and 15 complete\n')
