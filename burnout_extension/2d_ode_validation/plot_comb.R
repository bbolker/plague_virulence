...
mapping<-aes(R0-1,value,colour=factor(rho),linetype=factor(theta),
  group=interaction(factor(rho),factor(theta),K))
p<-ggplot(direct,mapping)+
  ...
  labs(title='Conditional persistence probability: full 2D ODE validation',
    subtitle='psi = 0.5; reused CTMC estimates; numerical finite-horizon Kendall prediction',
    x=expression(R[0]-1~'(log scale)'),y='Conditional persistence probability',
    caption='Colour identifies rho; line type and point shape identify theta = 0/0.5/1.\nCurves: numerical finite-horizon Kendall. Points and bars: reused CTMC estimates and 95% Wilson intervals.')+
...
cairo_pdf('2d_ode_validation/figures/fig13_psi05_2d_ode_comb.pdf',12,9,onefile=FALSE)
print(p);dev.off()
cat('2D ODE condensed figure complete\n')
