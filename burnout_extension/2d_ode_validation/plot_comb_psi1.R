...
direct<-copy(curves);direct[,value:=pmin(.9999,pmax(.0001,P_kendall))]
sim[,`:=`(estimate=pmin(.9999,pmax(.0001,P_conditional)),
  low=pmin(.9999,pmax(.0001,cond_low)),high=pmin(.9999,pmax(.0001,cond_high)))]
cols<-c('0.01'='#0072B2','0.05'='#D55E00','0.1'='#009E73')
p<-ggplot(direct,aes(R0-1,value,colour=factor(rho),linetype=factor(theta),
    group=interaction(factor(rho),factor(theta),K)))+
  geom_line(linewidth=.9,alpha=.9,na.rm=TRUE)+
  geom_pointrange(data=sim,aes(R0-1,y=estimate,ymin=low,ymax=high,
    colour=factor(rho),shape=factor(theta)),inherit.aes=FALSE,linewidth=.38,size=.9)+
  ...
  labs(title='Conditional persistence probability: full 2D ODE validation',
    subtitle='psi = 1; newly simulated CTMC estimates; numerical finite-horizon Kendall prediction',
    x=expression(R[0]-1~'(log scale)'),y='Conditional persistence probability',
    caption='Colour identifies rho; line type and point shape identify theta = 0/0.5/1.\nCurves: numerical finite-horizon Kendall where defined. Points and bars: new CTMC estimates and 95% Wilson intervals.')+
...
cairo_pdf('2d_ode_validation/figures/fig13_psi1_2d_ode_comb.pdf',12,9,onefile=FALSE)
print(p);dev.off()
cat('psi=1 condensed figure complete\n')
