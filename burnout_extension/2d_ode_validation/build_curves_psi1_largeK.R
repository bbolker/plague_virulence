...
stopifnot(requireNamespace('data.table',quietly=TRUE));library(data.table)
sim<-fread('2d_ode_validation/data/psi1_largeK_tau_results.csv')
dense<-CJ(rho=c(.01,.02,.05,.10),theta=c(0,.5,1),K=c(1e6,1e7,1e8,1e9),
  R0=1+exp(seq(log(.05),log(5),length.out=101)))
dense[,psi:=1];dense[,R0:=round(R0,12)];sim[,R0:=round(R0,12)]
grid<-unique(rbindlist(list(dense,sim[,.(rho,theta,K,R0,psi)])));setorder(grid,rho,theta,K,R0)
ans<-vector('list',nrow(grid))
for(i in seq_len(nrow(grid))){g<-grid[i]
...
curves[theta==0&status=='BAD_CROSSING_DIRECTION'&abs(alpha_t)<1e-7&
  abs(alpha_plus)<1e-7,status:='NO_TROUGH']
curves[,`:=`(P_trough_unconditional=(1-1/R0)*P_trough,
  P_kendall_unconditional=(1-1/R0)*P_kendall)]
fwrite(curves,'2d_ode_validation/data/psi1_largeK_2d_ode_curves.csv')
cat('psi=1 large-K 2D ODE and Kendall curves complete\n')
