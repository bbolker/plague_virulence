...
Raising it increases the parameter range with a usable crossing and may reduce
finite-$K$ entry noise, but may also reduce accuracy. The absence of a
deterministic crossing does not imply that the probability of burnout during
the first trough is negligible.
...
choice. Figures~\ref{fig:bl-unconditional-small} and
\ref{fig:bl-unconditional-large} show unconditional persistence, and
Figure~\ref{fig:bl-conditional} shows persistence conditional on escaping
early fizzle.
...
\footnotesize (d) $\BL=y^*$
\end{minipage}
\caption{Unconditional persistence: $\rho=0.05$, $\theta=0$, $K=1000$.}
\label{fig:bl-unconditional-small}
\end{figure}
...
\footnotesize (d) $\BL=y^*$
\end{minipage}
\caption{Unconditional persistence: $\rho=0.01$, $\theta=0$, $K=10^6$.}
\label{fig:bl-unconditional-large}
\end{figure}
...
\footnotesize (d) $\BL=y^*$
\end{minipage}
\caption{Persistence conditional on escaping early fizzle: $\rho=0.05$,
$\theta=0$, $K=1000$.}
\label{fig:bl-conditional}
\end{figure}

Across these examples, lower matching heights are more accurate where they can
be used, but their usable parameter range is smaller. Higher choices cover a
wider range but can have larger error; the $2/3$ and $3/4$ choices provide
