...
\title{Epidemic Burnout under Susceptible Recruitment and Population-Size-Dependent Transmission\\[0.4em]\large A Nonlinear Tail-Matching Theory from the First Epidemic Wave to Kendall Persistence Probability}
\author{Theory note}
\date{2026-09-08}
...
\]
Because $h_\theta(1)=0$, the endpoint conditions
\[
Y_1(1)=0,
\qquad
Y_2(1)=0
\]
are understood by continuous extension from the interior. The equations are integrated only to $\bar x>\xf$, where every coefficient is regular.
...
\mathcal I
=\int_0^\infty
\exp\left\{-\int_0^t[b(u)-d(u)]\dd u\right\}d(t)\dd t.
\]
Here $d(t)=1$. Along the susceptible recovery path,
\[
\dd\sigma=\frac{\dd X}{\rho\hth(X)}.
\]
Moreover,
\[
\int_{0}^{\sigma(X)}[b(u)-d(u)]\dd u
=\int_{x_h}^{X}
...
\subsection{Large \texorpdfstring{$R_0$}{R0} and positive \texorpdfstring{$\psi$}{psi}}
...
\end{aligned}
\]
Here the numerical coefficients follow from the Gaussian moments
\[
\langle t^2\rangle=1,
\qquad
\langle t^4\rangle=3,
...
\Apsi'(x)=\frac{r(x)}{h(x)}.
\]
At $x=\xs$ we have $r(\xs)=0$ and $\Rzero\xs^\eta=1$. The derivatives of $r$ at the saddle are
\[
r_*'=-\frac{\eta}{\xs},
\qquad
r_*''=-\frac{\eta(\eta-1)}{\xs^2},
\qquad
r_*'''=-\frac{\eta(\eta-1)(\eta-2)}{\xs^3}.
\]
Consequently
\[
