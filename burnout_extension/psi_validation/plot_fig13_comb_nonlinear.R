...
curves[,value:=pmin(.9999,pmax(.0001,leading))]
...
p<-ggplot(curves,aes(R0-1,value,colour=factor(rho),linetype=factor(theta),group=interaction(factor(rho),factor(theta),K)))+
  geom_line(linewidth=.85,na.rm=TRUE,alpha=.85)+
  geom_pointrange(data=sim,aes(R0-1,y=estimate,ymin=low,ymax=high,colour=factor(rho),shape=factor(theta)),inherit.aes=FALSE,linewidth=.38,size=.65,fatten=1.8)+
  ...
  labs(title='Conditional persistence probability: condensed nonlinear-tail validation',subtitle='psi = 0.5; reused stochastic estimates; nonlinear-tail action amplitude and leading Laplace approximation',x=expression(R[0]-1~'(log scale)'),y='Conditional persistence probability',caption='Colour identifies rho. Solid/22/44 curves identify theta = 0/0.5/1; circle/triangle/square points identify the same theta values. Curves are omitted outside the admissible descending-tail domain.')+
  ...
  guides(colour=guide_legend(order=1),linetype=guide_legend(order=2,override.aes=list(linewidth=1.2)),shape=guide_legend(order=2,override.aes=list(size=.65),nrow=1))
