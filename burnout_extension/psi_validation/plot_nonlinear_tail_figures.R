...
draw_leading<-function(probability,rho_value,theta_value){
  a<-melt(copy(curves[rho==rho_value&theta==theta_value]),
    id.vars=c('rho','theta','K','R0'),measure.vars=c('leading','trough'),
    variable.name='method',value.name='value')
  a[,method:=factor(method,c('leading','trough'),
    c('Low-prevalence action','Finite-prevalence trough'))]
  s<-copy(sim[rho==rho_value&theta==theta_value])
  if(probability=='unconditional'){
    a[,value:=value*(1-1/R0)];s[,`:=`(estimate=P_unconditional,low=uncond_low,high=uncond_high)]
  ...
  p<-ggplot()+geom_line(data=a,aes(R0-1,value,colour=method),linewidth=.9,na.rm=TRUE)+
    ...
    scale_colour_manual(values=c('Low-prevalence action'='#009E73','Finite-prevalence trough'='#CC79A7'))+
    labs(title=ttl,subtitle=sprintf('Nonlinear-tail and trough comparison; psi = 0.5, rho = %.2f, theta = %g; I0 = 1',rho_value,theta_value),
      x=expression(R[0]-1~'(log scale)'),y=yl,colour=NULL,
      caption='Blue points and grey bars: reused CTMC estimates and 95% Wilson intervals. Green: action/Laplace; purple: finite-prevalence Kendall trough.\nCurves stop where the regular-outer descending initialization is unavailable.')+theme_nl
  if(probability=='unconditional'){
...
draw_next<-function(rho_value,theta_value){
  a<-melt(curves[rho==rho_value&theta==theta_value],
    id.vars=c('rho','theta','K','R0'),measure.vars=c('leading','next_order','trough'),
    variable.name='method',value.name='value')
  a[,method:=factor(method,c('leading','next_order','trough'),
    c('Leading nonlinear tail','Next Laplace correction','Finite-prevalence trough'))]
  ...
  ggplot()+geom_line(data=a,aes(R0-1,value,colour=method),linewidth=.9,na.rm=TRUE)+
    ...
    scale_colour_manual(values=c('Leading nonlinear tail'='#009E73','Next Laplace correction'='#D55E00',
      'Finite-prevalence trough'='#CC79A7'))+
    labs(title='Conditional persistence probability',subtitle=sprintf('Nonlinear-tail comparison; psi = 0.5, rho = %.2f, theta = %g; I0 = 1',rho_value,theta_value),
      ...
      caption='Blue points and grey bars: reused CTMC estimates and 95% Wilson intervals. Green: leading nonlinear tail; orange: exp(-rho*cL) correction; purple: finite-prevalence trough.')+theme_nl
}
