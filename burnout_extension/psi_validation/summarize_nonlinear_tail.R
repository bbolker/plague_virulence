...
at[,`:=`(leading=approx(log(curves[rho==.BY$rho&theta==.BY$theta&K==.BY$K,R0]-1),
  ...
  curves[rho==.BY$rho&theta==.BY$theta&K==.BY$K,next_order],log(R0_sim-1),rule=1)$y,
  trough=approx(log(curves[rho==.BY$rho&theta==.BY$theta&K==.BY$K,R0]-1),
  curves[rho==.BY$rho&theta==.BY$theta&K==.BY$K,trough],log(R0_sim-1),rule=1)$y),by=.(rho,theta,K)]
setnames(at,'R0_sim','R0');z<-merge(simdt,at,by=c('rho','theta','K','R0'))
out<-z[,.(points=.N,defined_leading=sum(is.finite(leading)),
  defined_trough=sum(is.finite(trough)),
  ...
  MAE_trough_conditional=mean(abs(P_conditional-trough),na.rm=TRUE))]
diag<-curves[,.(fraction_nominal_overlap=mean(overlap_score<=1,na.rm=TRUE),
  ...
  median_trough_relative_prevalence=median(s_t,na.rm=TRUE),
  fraction_finite_prevalence_trough=mean(s_t>=.1,na.rm=TRUE))]
