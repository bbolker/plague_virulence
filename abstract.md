...
@LensMay1994 published one of the first models of the virulence-transmission tradeoff. However, even though plague is the first example they cite in their introduction, they assumed (as is typical in the theoretical literature) that (1) hosts transmit disease throughout their infectious periods, and (2) transmission *rate* is an accelerating function of the *rate* of host mortality (their definition of 'virulence'). Thus their model formulation doesn't match the natural history of plague.
...
* probably the main thing holding us back from exploring these hypotheses is htat spatial models are significantly more complicated, harder to construct, and computationally slower, than the models we need to explore fast-slow mechanisms ...
