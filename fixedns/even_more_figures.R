...
pdf("critical_region.pdf", width=12, height=12)

for (c0_val in c(0.25, 0.5, 1)) {
  for (r_val in c(0.25, 0.5, 1)) {
    
    par(mfrow=c(5,5), mar=c(3,3,2,1), oma=c(2,2,3,2))  
    
    for (s in S_vals) {
      for (b in B0_vals) {
        
        # canPersist function
        canPersist_curr <- function(c0=c0_val,
                                    S=s,
                                    B0=b,
                                    R0=2.5,
                                    epsilon=0.05,
        ...
        matter_curr <- function(epsilon=0.05, R0=3){
          return(canPersist_curr(D=1,R0=R0,epsilon=epsilon) != 
                   canPersist_curr(D=0.85,R0=R0,epsilon=epsilon))
        }
        ...
        # Plot subfigure
        image(epsvec, R0vec, M, 
              col=c("#FFFFFF", adjustcolor("blue", alpha=0.3)),
              xlab="epsilon", ylab="R0", 
              main=paste0("S=",s,", B0=",b))
      }
    }
    
    # Add title to the whole page
    mtext(paste0("c0=", c0_val, ", r=", r_val), 
